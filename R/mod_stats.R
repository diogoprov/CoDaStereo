  ...
  tabItem(tabName = "stats",
    ...
    fluidRow(
      box(title = "Pairwise comparisons", width = 6, status = "info",
          ...
          actionButton(ns("run_pairwise"), "Run Pairwise",
...
mod_stats_server <- function(id, df, parts, comp, keep_rows) {
  moduleServer(id, function(input, output, session) {
    ...
    # -- Pairwise group selector -----------------------------------------------
    # Defaults to the first MANOVA factor (most common use case).
    ...
    observeEvent(input$run_manova, {
      ...
      factor_list <- lapply(input$factors, function(f) {
        as.factor(df()[[f]][keep_rows()])
      })
    ...
    output$diag_resid <- renderPlot({
      req(rv$model)
      plot(rv$model, type = "diagnostics")
    })

    output$diag_pc <- renderPlot({
      req(rv$model)
      plot(rv$model, type = "PC", pch = 19)
    })
    ...
    output$dl_pairwise <- downloadHandler(
      ...
      content  = function(file) {
        req(rv$pairwise_out)
        pw_summary <- summary(rv$pairwise_out, test.type = "dist")
        # Extract the summary table
        tbl <- pw_summary$summary.table
        utils::write.csv(tbl, file)
      }
    )
  })
